...
class EnvRunner():
    ...
    def run(self):
        ...
        for episode in range(self.episodes):
            ...
            if (episode + 1) % self.train_every_step == 0:
                self.train()
                self.buffers = [[] for _ in range(self.num_agent)]
                #rnn_states = [None for _ in range(self.num_agent)]
                self.update_par(episode + 1, self.episodes)

                if (episode + 1) % self.update_eval_every_step == 0:
                    self.eval_update()
                    if (episode + 1) % self.save_eval_every_step == 0:
                        self.save_model()

            print(f"Iteration: {episode + 1} / {self.episodes}")
