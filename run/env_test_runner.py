...
class EnvRunner():
    ...
    def run(self):
        ...
        for episode in range(self.episodes):
            ...
            for step in range(self.episode_length):
                ...
                _, actions, rnn_states = self.collect(obs, rnn_states, deterministic=True)             # collect actions and corresponding probs
                obs_next, rewards = self.env.step(actions, step)

                #self.push(q_values, obs_next, rnn_states, rewards, step)
                obs = copy.copy(obs_next)
                throughput_his.append(copy.copy(self.env.get_short_term_throughput_for_each()))

            Throughput_Ret = {'Throughput_Ret': throughput_his}
            f_name = 'Throughput_Ret_' + 'N' + str(self.num_agent) + 'K' + str(self.num_channel) + '.mat'
    ...
    def collect(self, obs, rnn_states,deterministic=False):
        actions = []
        next_rnn_states = []
        q_values = []
        curr_branch = min(self.num_agent, self.max_num_branch)
        for agent in range(self.num_agent):
            q_value, action, rnn_state = self.agents[agent].select_action(obs[agent], curr_branch, rnn_states[agent], deterministic)
